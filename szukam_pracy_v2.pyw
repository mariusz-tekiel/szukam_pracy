...
os.environ["JOOBLE_API_KEY"] = "13a3403d-c2c7-4bfa-8624-7b1ad44412f4"
...
def fetch_jooble(job_title: str, work_type: str, city: Optional[str], max_years: int) -> List[JobOffer]:
    ...
    if wt == "remote":
        ...
        keywords += " hybrid"
    location = city if city and wt != "remote" else ""
    payload = {"keywords": keywords, "location": location, "ResultOnPage": 50, "page": 1}
    try:
        r = requests.post(
    ...
    keywords_lower = [kw.lower() for kw in job_title.split() if kw]
    remote_kw = re.compile(r"\b(remote|zdaln|home\s*office)\b", re.I)
    hybrid_kw = re.compile(r"\b(hybrid|hybryd)\b", re.I)

    out: List[JobOffer] = []
    for it in data.get("jobs", []):
        title = it.get("title") or ""
        desc_raw = re.sub(r"<[^>]+>", "", it.get("snippet") or "")
        combined = f"{title} {desc_raw}".lower()

        # filtr tytułu — przynajmniej jedno słowo kluczowe musi pasować
        if not any(kw in combined for kw in keywords_lower):
            continue

        # filtr trybu pracy
        if wt == "remote" and not remote_kw.search(combined):
            continue
        if wt == "hybrydowa" and not hybrid_kw.search(combined):
            continue

        created = parse_iso(it.get("updated", ""))
        salary_str = (it.get("salary") or "").replace("\xa0", "").replace(" ", "")
        salary_min = salary_max = None
        currency = None
        m = re.search(r"(\d[\d.,]*)\D+(\d[\d.,]*)", salary_str)
        if m:
        ...
        offer = JobOffer(
            ...
            company=it.get("company") or "N/D",
            location=it.get("location") or "N/D",
            url=it.get("link") or "",
            source="Jooble",
            created_at=created if created.tzinfo else created.replace(tzinfo=dt.timezone.utc),
            salary_min=salary_min,
...
def fetch_arbeitnow(job_title: str, work_type: str, max_years: int) -> List[JobOffer]:
    params = {"search": job_title, "page": 1}
    wt = work_type.lower()
    if wt == "remote":
        params["remote"] = "true"
    try:
    ...
    for it in data.get("data", []):
        is_remote = it.get("remote", False)

        # filtr trybu pracy
        if wt == "remote" and not is_remote:
            continue
        if wt in ("hybrydowa", "stacjonarna") and is_remote:
            continue
        ...
        created = dt.datetime.fromtimestamp(it.get("created_at", 0), tz=dt.timezone.utc)
        offer = JobOffer(
            title=title or "Brak tytułu",
            company=it.get("company_name") or "N/D",
            location=it.get("location") or "EU",
            url=it.get("url") or "",
            source="Arbeitnow",
            created_at=created,
            description=desc[:4000],
            meta={"tags": it.get("tags", [])},
        )
        offer.is_stale = mark_stale(offer.created_at)
        if not requires_more_than(offer.title, offer.description, max_years):
            out.append(offer)
...
class App(tk.Tk):
    ...
    def _do_search(self, job: str, work_type: str, city: Optional[str], max_years: int):
        ...
        if work_type.lower() == "remote":
            ...
            self.set_progress(25, f"Z Remotive: {len(offers)}")
        self.set_progress(30, "Pobieram (Adzuna)...")
        offers += fetch_adzuna(job, work_type, city, max_years)
        self.set_progress(55, f"Z Adzuna: {len(offers)}")
        self.set_progress(60, "Pobieram (Jooble)...")
        offers += fetch_jooble(job, work_type, city, max_years)
        self.set_progress(70, f"Z Jooble: {len(offers)}")
        self.set_progress(75, "Pobieram (Arbeitnow)...")
        offers += fetch_arbeitnow(job, work_type, max_years)
        self.set_progress(88, f"Po zebraniu i filtrach: {len(offers)}")
